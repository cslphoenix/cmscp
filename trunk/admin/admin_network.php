...
else
{
	...
	if ( $mode )
	{
		switch ( $mode )
		{
			...
			case '_update':
				...
				if ( request('submit', 1) )
				{
					$network_img	= request_file('network_image');
					...
					if ( !$error )
					{
						...
						else
						{
							...
							$msg = sprintf($lang['update'], $network_info) . sprintf($lang['return_update'], check_sid($file), $acp_title, check_sid("$file?mode=$mode&$url=$data_id"));
						}
	...
	$template->assign_vars(array(
		...
		'L_CREATE_LINK'		=> sprintf($lang['sprintf_new_createn'], $lang['link']),
		'L_CREATE_PARTNER'	=> sprintf($lang['sprintf_new_createn'], $lang['partner']),
		'L_CREATE_SPONSOR'	=> sprintf($lang['sprintf_new_createn'], $lang['sponsor']),
	...
	if ( $tmp_link )
	{
		for ( $i = $start; $i < min($settings['site_entry_per_page'] + $start, count($tmp_link)); $i++ )
		{
			...
			$template->assign_block_vars('_display._link_row', array(
				...
				'U_UPDATE'	=> check_sid("$file?mode=_update&amp;$url=$network_id"),
				'U_DELETE'	=> check_sid("$file?mode=_delete&amp;$url=$network_id"),
			));
		}
	}
	else
