...
if( !empty($setmodules) )
{
	...
	$module['Styles']['Add_new'] = "$root_file?mode=addnew";
	$module['Styles']['Create_new'] = "$root_file?mode=create";
	$module['Styles']['Manage'] = $root_file;
	$module['Styles']['Export'] = "$root_file?mode=export";
	return;
}
...
if ($cancel)
{
	redirect('admin/' . check_sid("admin_styles.$phpEx", true));
}
...
switch( $mode )
{
	case "addnew":
		...
		if( isset($install_to) )
		{
			...
			$message = $lang['Theme_installed'] . "<br /><br />" . sprintf($lang['Click_return_styleadmin'], "<a href=\"" . check_sid("admin_styles.$phpEx") . "\">", "</a>") . "<br /><br />" . sprintf($lang['Click_return_admin_index'], "<a href=\"" . check_sid("index.$phpEx?pane=right") . "\">", "</a>");
		...
		else
		{
			...
			if( $dir = @opendir($phpbb_root_path. "templates/") )
			{
				while( $sub_dir = @readdir($dir) )
				{
					...
					if( !is_file(cms_realpath($phpbb_root_path . 'templates/' .$sub_dir)) && !is_link(cms_realpath($phpbb_root_path . 'templates/' .$sub_dir)) && $sub_dir != "." && $sub_dir != ".." && $sub_dir != "CVS" )
					{
						if( @file_exists(@cms_realpath($phpbb_root_path. "templates/" . $sub_dir . "/theme_info.cfg")) )
						{
				...
				$template->set_filenames(array(
					"body" => "style/styles_addnew_body.tpl")
				);
				...
				for($i = 0; $i < count($installable_themes); $i++)
				{
					...
					$template->assign_block_vars("styles", array(
						...
						"U_STYLES_INSTALL" => check_sid("admin_styles.$phpEx?mode=addnew&amp;style=" . urlencode($installable_themes[$i]['style_name']) . "&amp;install_to=" . urlencode($installable_themes[$i]['template_name'])))
					);
	...
	case "edit":
		...
		if( $submit )
		{
			...
			//
			$submitd['style_name'] = $HTTP_POST_VARS['style_name'];
			$submitd['template_name'] = $HTTP_POST_VARS['template_name'];
			$submitd['head_stylesheet'] = $HTTP_POST_VARS['head_stylesheet'];
			...
			if($mode == "edit")
			{
				...
				$message = $lang['Theme_updated'] . "<br /><br />" . sprintf($lang['Click_return_styleadmin'], "<a href=\"" . check_sid("admin_styles.$phpEx") . "\">", "</a>") . "<br /><br />" . sprintf($lang['Click_return_admin_index'], "<a href=\"" . check_sid("index.$phpEx?pane=right") . "\">", "</a>");
			...
			else
			{
				...
				$sql = "SELECT themes_id 
					FROM " . THEMES . " 
					WHERE style_name = '" . str_replace("\'", "''", $submitd['style_name']) . "'";
				if(!$result = $db->sql_query($sql))
				{
					message(GENERAL_ERROR, "Could not query themes table", "", __LINE__, __FILE__, $sql);
				}
				...
				$message = $lang['Theme_created'] . "<br /><br />" . sprintf($lang['Click_return_styleadmin'], "<a href=\"" . check_sid("admin_styles.$phpEx") . "\">", "</a>") . "<br /><br />" . sprintf($lang['Click_return_admin_index'], "<a href=\"" . check_sid("index.$phpEx?pane=right") . "\">", "</a>");
		...
		else
		{
			...
			{	
				...
				{	
					if( !is_file(cms_realpath($phpbb_root_path . 'templates/' . $file)) && !is_link(cms_realpath($phpbb_root_path . 'templates/' . $file)) && $file != "." && $file != ".." && $file != "CVS" )
					{
			...
			$template->assign_vars(array(
				...
				"S_THEME_ACTION" => check_sid("admin_styles.$phpEx"),
	...
	case "export";
		if($HTTP_POST_VARS['export_template'])
		{
			...
			if( !$fp )
			{
				...
				$download_form = '<form action="' . check_sid("admin_styles.$phpEx") . '" method="post"><input class="mainoption" type="submit" name="submit" value="' . $lang['Download'] . '" />' . $s_hidden_fields;
			...
			$message = $lang['Theme_info_saved'] . "<br /><br />" . sprintf($lang['Click_return_styleadmin'], "<a href=\"" . check_sid("admin_styles.$phpEx") . "\">", "</a>") . "<br /><br />" . sprintf($lang['Click_return_admin_index'], "<a href=\"" . check_sid("index.$phpEx?pane=right") . "\">", "</a>");
		...
		else
		{
			...
			$template->assign_vars(array(
				...
				"S_EXPORTER_ACTION" => check_sid("admin_styles.$phpEx?mode=export"),
				"S_TEMPLATE_SELECT" => $s_template_select)
			);
	...
	case "delete":
		...
		if( !$confirm )
		{
			...
			$template->assign_vars(array(
				...
				"S_CONFIRM_ACTION" => check_sid("admin_styles.$phpEx"),
		...
		else
		{
			...
			{
				message(GENERAL_ERROR, "Could not remove style data!", "", __LINE__, __FILE__, $sql);
			}
			...
			$message = $lang['Style_removed'] . "<br /><br />" . sprintf($lang['Click_return_styleadmin'], "<a href=\"" . check_sid("admin_styles.$phpEx") . "\">", "</a>") . "<br /><br />" . sprintf($lang['Click_return_admin_index'], "<a href=\"" . check_sid("index.$phpEx?pane=right") . "\">", "</a>");
	...
	default:
		...
		if(!$result = $db->sql_query($sql))
		{
			message(GENERAL_ERROR, "Could not get style information!", "", __LINE__, __FILE__, $sql);
		}
		...
		for($i = 0; $i < count($style_rowset); $i++)
		{
			...
			$template->assign_block_vars("styles", array(
				...
				"U_STYLES_EDIT" => check_sid("admin_styles.$phpEx?mode=edit&amp;style_id=" . $style_rowset[$i]['themes_id']),
				"U_STYLES_DELETE" => check_sid("admin_styles.$phpEx?mode=delete&amp;style_id=" . $style_rowset[$i]['themes_id']))
			);
		}
