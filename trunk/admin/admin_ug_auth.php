...
else
{
	...
	if ( isset($HTTP_POST_VARS['submit']) && ( ( $mode == 'user' && $user_id ) || ( $mode == 'group' && $group_id ) ) )
	{
		...
		//	$message = $lang['Auth_updated'] . '<br><br>' . sprintf($lang['Click_return_userauth'], '<a href="' . check_sid('admin_ug_auth.php?mode=$mode") . '">', '</a>') . '<br><br>' . sprintf($lang['Click_return_admin_index'], '<a href="' . check_sid('index.php?pane=right"));
		...
		else if ( $mode == 'user' && $HTTP_POST_VARS['userlevel'] == 'trial' && $user_level != TRIAL )
		{
			...
			if ( $userdata['user_id'] != $user_id )
			{
				...
				$sql = "DELETE FROM " . FORUM_ACCESS . " WHERE group_id = $group_id AND auth_mod = 0";
				if ( !($result = $db->sql_query($sql)) ) 
				{
					message(GENERAL_ERROR, "Couldn't delete auth access info", "", __LINE__, __FILE__, $sql); 
				} 
			...
			else
			{
				...
				while( list($forum_id, $action) = @each($forum_auth_action) )
				{
						...
						if ( $action == 'insert' )
					{
						...
						$sql = "INSERT INTO " . FORUM_ACCESS . " (forum_id, group_id, $sql_field) 
		...
		//		$message = $lang['Auth_updated'] . '<br><br>' . sprintf($l_auth_return, '<a href="' . check_sid('admin_ug_auth.php?mode=$mode") . '">', '</a>') . '<br><br>' . sprintf($lang['Click_return_admin_index'], '<a href="' . check_sid('index.php?pane=right"));
			}
			...
			$sql = "SELECT u.user_id 
				FROM " . FORUM_ACCESS . " aa, " . GROUPS_USERS . " ug, " . USERS . " u  
			...
			$sql = "SELECT u.user_id 
				FROM ( ( " . USERS . " u  
				LEFT JOIN " . GROUPS_USERS . " ug ON ug.user_id = u.user_id ) 
				LEFT JOIN " . FORUM_ACCESS . " aa ON aa.group_id = ug.group_id ) 
			...
			$sql = "SELECT ug.user_id, COUNT(auth_mod) AS is_auth_mod 
				FROM " . FORUM_ACCESS . " aa, " . GROUPS_USERS . " ug 
	...
	else if ( ( $mode == 'user' && ( isset($HTTP_POST_VARS['user_name']) || $user_id ) ) || ( $mode == 'group' && $group_id ) )
	{
		...
		while( list($forum_id, $user_ary) = @each($auth_ug) )
		{
			...
			$optionlist_mod = '<select class="postselect" name="moderator[' . $forum_id . ']">';
			...
			$template->assign_block_vars('forums', array(
				...
				'U_FORUM_AUTH' => check_sid('admin_forumauth.php?f=' . $forum_access[$i]['forum_id']),
	...
	//	@reset($auth_user);
		...
		if ( $mode == 'user' )
		{
			$t_user_name = $ug_info[0]['user_name'];
		...
		for($i = 0; $i < count($ug_info); $i++)
		{
			...
			{
				$name[] = ( $mode == 'user' ) ? $ug_info[$i]['group_name'] :  $ug_info[$i]['user_name'];
		...
		if( count($name) )
		{
			for($i = 0; $i < count($ug_info); $i++)
			{
				...
				{
					$t_usergroup_list .= ( ( $t_usergroup_list != '' ) ? ', ' : '' ) . '<a href="' . check_sid("admin_ug_auth.php?mode=$ug=" . $id[$i]) . '">' . $name[$i] . '</a>';
				}
				else
				{
					$t_pending_list .= ( ( $t_pending_list != '' ) ? ', ' : '' ) . '<a href="' . check_sid("admin_ug_auth.php?mode=$ug=" . $id[$i]) . '">' . $name[$i] . '</a>';
				}
			}
		}
		...
		$u_ug_switch = ( $mode == 'user' ) ? POST_USER . "=" . $user_id : POST_GROUPS . "=" . $group_id;
		$switch_mode = check_sid("admin_ug_auth.php?mode=$mode&amp;" . $u_ug_switch . "&amp;adv=$adv_switch");
		$switch_mode_text = ( empty($adv) ) ? $lang['Advanced_mode'] : $lang['Simple_mode'];
		$u_switch_mode = '<a href="' . $switch_mode . '">' . $switch_mode_text . '</a>';
	
		$fields = '<input type="hidden" name="mode" value="' . $mode . '" /><input type="hidden" name="adv" value="' . $adv . '" />';
		$fields .= ( $mode == 'user' ) ? '<input type="hidden" name="' . POST_USER . '" value="' . $user_id . '" />' : '<input type="hidden" name="' . POST_GROUPS . '" value="' . $group_id . '" />';
		...
		$template->assign_vars(array(
			...
			'U_USER_OR_GROUP' => check_sid('admin_ug_auth.php'),
			'U_SWITCH_MODE' => $u_switch_mode,
	
			'S_COLUMN_SPAN' => $s_column_span,
			'S_AUTH_ACTION' => check_sid('admin_ug_auth.php'), 
			'S_FIELDS' => $fields)
		);
	}
	else
	{
		...
		if ( $mode == 'user' )
		{
			...
			{
				$u_select_list = '<select class="select" name="' . POST_USER . '">';
				do
				{
					$u_select_list .= '<option value="' . $row['user_id'] . '">' . $row['user_name'] . '</option>';
				}
				while ( $row = $db->sql_fetchrow($result) );
				$u_select_list .= '</select>';
			}
			
			$template->assign_vars(array(
				'S_USERS_SELECT' => $u_select_list
			));
		}
		else
		{
			...
			if ( $row = $db->sql_fetchrow($result) )
			{
				$g_select_list = '<select class="select" name="' . POST_GROUPS . '">';
				do
				{
					$g_select_list .= '<option value="' . $row['group_id'] . '">' . $row['group_name'] . '</option>';
				}
				while ( $row = $db->sql_fetchrow($result) );
				$g_select_list .= '</select>';
			}
	
			$template->assign_vars(array(
				'S_GROUPS_SELECT' => $g_select_list
			));
		}
		...
		$template->assign_vars(array(
			...
			'S_FIELDS' => $fields, 
			'S_' . $l_type . '_ACTION' => check_sid('admin_ug_auth.php'))
		);
