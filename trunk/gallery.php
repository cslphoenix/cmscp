...
#	debug($gallery);
	...
	if ( $userdata['user_level'] < $gallery['auth_view'] )
	{
		...
		{
			redirect(check_sid('login.php?redirect=gallery.php&id=' . $data_id, true));
		}
		else
		{
...
#	debug($s_auth_can);
	...
	if ( $galleries_pics )
	{
		...
		$template->assign_vars(array(
			...
			'PAGINATION'	=> generate_pagination('gallery.php?mode=_overview&id=' . $data_id, count($galleries_pics), $gallery['per_cols'], $start),
...
#	debug($galleries);
	...
	for ( $i = 0; $i < count($galleries); $i++ )
	{
		...
		$template->assign_block_vars('select.row_galleries', array(
			...
			'U_GALLERY'			=> check_sid('gallery.php?id=' . $galleries[$i]['gallery_id']),
#			'TEAM_FIGHTUS'	=> ( $teams[$j]['team_fight'] )	? '<a href="' . check_sid('contact.php?mode=fightus&amp;' . POST_TEAMS . '=' . $team_id) . '">' . $lang['match_fightus'] . '</a>'  : '',
#			'TO_TEAM'		=> check_sid('gallery.php?mode=view&amp;id=' . $teams[$j]['team_id']),
		));
	}
}	
