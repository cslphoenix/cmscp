...
if ( $mode == 'topicreview' )
{
	require($root_path . 'includes/topic_review.php');
...
if ( isset($HTTP_POST_VARS['cancel']) )
{
	...
	redirect(check_sid($redirect, true) . $post_append);
}
...
switch( $mode )
{
	...
	case 'topicreview':
		...
		break;
	default:
		message(GENERAL_MESSAGE, $lang['No_post_mode']);
		break;
}
...
switch ( $mode )
{
	case 'newtopic':
		if ( empty($forum_id) )
		{
			message(GENERAL_MESSAGE, $lang['Forum_not_exist']);
		}
	...
	case 'vote':
		if ( empty( $topic_id) )
		{
			message(GENERAL_MESSAGE, $lang['No_topic_id']);
		}

		$sql = "SELECT f.*, t.topic_status, t.topic_title, t.topic_type  
			FROM " . FORUM . " f, " . TOPICS_TABLE . " t
	...
	case 'poll_delete':
		if ( empty($post_id) )
		{
			message(GENERAL_MESSAGE, $lang['No_post_id']);
		}

		$select_sql = (!$submit) ? ', t.topic_title, p.enable_bbcode, p.enable_html, p.enable_smilies, p.enable_sig, p.post_user_name, pt.post_subject, pt.post_text, pt.bbcode_uid, u.user_name, u.user_id, u.user_sig, u.user_sig_bbcode_uid' : '';
		$from_sql = ( !$submit ) ? ", " . POSTS_TEXT_TABLE . " pt, " . USERS_TABLE . " u" : '';
		$where_sql = ( !$submit ) ? "AND pt.post_id = p.post_id AND u.user_id = p.poster_id" : '';

		$sql = "SELECT f.*, t.topic_id, t.topic_status, t.topic_type, t.topic_first_post_id, t.topic_last_post_id, t.topic_vote, p.post_id, p.poster_id" . $select_sql . " 
			FROM " . POSTS_TABLE . " p, " . TOPICS_TABLE . " t, " . FORUM . " f" . $from_sql . " 
	...
	default:
		message(GENERAL_MESSAGE, $lang['No_valid_mode']);
}
...
if ( ($result = $db->sql_query($sql)) && ($post_info = $db->sql_fetchrow($result)) )
{
	...
	{ 
	   message(GENERAL_MESSAGE, $lang['Forum_locked']); 
	} 
	else if ( $mode != 'newtopic' && $post_info['topic_status'] == TOPIC_LOCKED && !$is_auth['auth_mod']) 
	{ 
	   message(GENERAL_MESSAGE, $lang['Topic_locked']); 
	} 
	...
	if ( $mode == 'editpost' || $mode == 'delete' || $mode == 'poll_delete' )
	{
		...
		if ( $post_data['first_post'] && $post_data['has_poll'] )
		{
			...
			{
				message(GENERAL_ERROR, 'Could not obtain vote data for this topic', '', __LINE__, __FILE__, $sql);
			}
		...
		if ( $post_info['poster_id'] != $userdata['user_id'] && !$is_auth['auth_mod'] )
		{
			...
			$message .= '<br /><br />' . sprintf($lang['Click_return_topic'], '<a href="' . check_sid("viewtopic.php?" . POST_TOPIC_URL . "=$topic_id") . '">', '</a>');

			message(GENERAL_MESSAGE, $message);
		}
		else if ( !$post_data['last_post'] && !$is_auth['auth_mod'] && ( $mode == 'delete' || $delete ) )
		{
			message(GENERAL_MESSAGE, $lang['Cannot_delete_replied']);
		}
		else if ( !$post_data['edit_poll'] && !$is_auth['auth_mod'] && ( $mode == 'poll_delete' || $poll_delete ) )
		{
			message(GENERAL_MESSAGE, $lang['Cannot_delete_poll']);
		}
	}
	else
	{
	...
	{
		message(GENERAL_MESSAGE, $lang['No_such_post']);
	}
}
else
{
...
if ( !$is_auth[$is_auth_type] )
{
	...
	{
		message(GENERAL_MESSAGE, sprintf($lang['Sorry_' . $is_auth_type], $is_auth[$is_auth_type . "_type"]));
	}
	...
	redirect(check_sid("login.php?redirect=posting.php&" . $redirect, true));
}
...
{
	$html_on = ( $submit || $refresh ) ? ( ( !empty($HTTP_POST_VARS['disable_html']) ) ? 0 : TRUE ) : ( ( $userdata['user_id'] == ANONYMOUS ) ? $config['allow_html'] : $userdata['user_allowhtml'] );
}
...
{
	$bbcode_on = ( $submit || $refresh ) ? ( ( !empty($HTTP_POST_VARS['disable_bbcode']) ) ? 0 : TRUE ) : ( ( $userdata['user_id'] == ANONYMOUS ) ? $config['allow_bbcode'] : $userdata['user_allowbbcode'] );
}
...
{
	$smilies_on = ( $submit || $refresh ) ? ( ( !empty($HTTP_POST_VARS['disable_smilies']) ) ? 0 : TRUE ) : ( ( $userdata['user_id'] == ANONYMOUS ) ? $config['allow_smilies'] : $userdata['user_allowsmile'] );
}
*/
...
else
{
	if ( $mode != 'newtopic' && $userdata['session_logged_in'] && $is_auth['auth_read'] )
	{
		...
		if ( !($result = $db->sql_query($sql)) )
		{
			message(GENERAL_ERROR, 'Could not obtain topic watch information', '', __LINE__, __FILE__, $sql);
		}
...
if ( ( $delete || $poll_delete || $mode == 'delete' ) && !$confirm )
{
	...
	$template->assign_vars(array(
		...
		'S_CONFIRM_ACTION' => check_sid("posting.php"),
...
else if ( $mode == 'vote' )
{
	...
	if ( !empty($HTTP_POST_VARS['vote_id']) )
	{
		...
		if ( $vote_info = $db->sql_fetchrow($result) )
		{
			...
			{
				message(GENERAL_ERROR, 'Could not obtain user vote data for this topic', '', __LINE__, __FILE__, $sql);
			}
			...
			if ( !($row = $db->sql_fetchrow($result2)) )
			{
				...
				{
					message(GENERAL_ERROR, 'Could not update poll result', '', __LINE__, __FILE__, $sql);
				}
				...
				{
					message(GENERAL_ERROR, "Could not insert user_id for poll", "", __LINE__, __FILE__, $sql);
				}
		...
		$template->assign_vars(array(
			'META' => '<meta http-equiv="refresh" content="3;url=' . check_sid("viewtopic.php?" . POST_TOPIC_URL . "=$topic_id") . '">')
		);
		$message .=  '<br /><br />' . sprintf($lang['Click_view_message'], '<a href="' . check_sid("viewtopic.php?" . POST_TOPIC_URL . "=$topic_id") . '">', '</a>');
		message(GENERAL_MESSAGE, $message);
	}
	else
	{
		redirect(check_sid("viewtopic.php?" . POST_TOPIC_URL . "=$topic_id", true));
	}
}
else if ( $submit || $confirm )
{
	...
	switch ( $mode )
	{
		...
		case 'newtopic':
		case 'reply':
			$user_name = ( !empty($HTTP_POST_VARS['user_name']) ) ? $HTTP_POST_VARS['user_name'] : '';
			...
			prepare_post($mode, $post_data, $bbcode_on, $html_on, $smilies_on, $error_msg, $user_name, $bbcode_uid, $subject, $message, $poll_title, $poll_options, $poll_length);

			if ( $error_msg == '' )
			{
				$topic_type = ( $topic_type != $post_data['topic_type'] && !$is_auth['auth_sticky'] && !$is_auth['auth_announce'] ) ? $post_data['topic_type'] : $topic_type;

				submit_post($mode, $post_data, $return_message, $return_meta, $forum_id, $topic_id, $post_id, $poll_id, $topic_type, $bbcode_on, $html_on, $smilies_on, $attach_sig, $bbcode_uid, str_replace("\'", "''", $user_name), str_replace("\'", "''", $subject), str_replace("\'", "''", $message), str_replace("\'", "''", $poll_title), $poll_options, $poll_length);
			}
			break;
		...
		case 'poll_delete':
			if ($error_msg != '')
			{
				message(GENERAL_MESSAGE, $error_msg);
			}
	...
	if ( $error_msg == '' )
	{
		...
		{
			$tracking_topics = ( !empty($HTTP_COOKIE_VARS[$config['cookie_name'] . '_t']) ) ? unserialize($HTTP_COOKIE_VARS[$config['cookie_name'] . '_t']) : array();
			$tracking_forums = ( !empty($HTTP_COOKIE_VARS[$config['cookie_name'] . '_f']) ) ? unserialize($HTTP_COOKIE_VARS[$config['cookie_name'] . '_f']) : array();
		...
		message(GENERAL_MESSAGE, $return_message);
	}
}

if( $refresh || isset($HTTP_POST_VARS['del_poll_option']) || $error_msg != '' )
{
	$user_name = ( !empty($HTTP_POST_VARS['user_name']) ) ? htmlspecialchars(trim(stripslashes($HTTP_POST_VARS['user_name']))) : '';
	...
	if( $preview )
	{
		...
		if( !empty($orig_word) )
		{
			$preview_user_name = ( !empty($user_name) ) ? preg_replace($orig_word, $replacement_word, $preview_user_name) : '';
...
else
{
	...
	if ( $mode == 'newtopic' )
	{
		...
		$user_name = ($userdata['session_logged_in']) ? $userdata['user_name'] : '';
		$poll_title = '';
		$poll_length = '';
		$subject = '';
		$message = '';
	...
	else if ( $mode == 'quote' || $mode == 'editpost' )
	{
		...
		if ( $mode == 'quote' )
		{
			...
			$msg_date =  create_date($config['default_dateformat'], $postrow['post_time'], $config['board_timezone']);

			// Use trim to get rid of spaces placed there by MS-SQL 2000
			$quote_user_name = ( trim($post_info['post_user_name']) != '' ) ? $post_info['post_user_name'] : $post_info['user_name'];
			$message = '[quote="' . $quote_user_name . '"]' . $message . '[/quote]';
		...
		{
			$user_name = ( $post_info['user_id'] == ANONYMOUS && !empty($post_info['post_user_name']) ) ? $post_info['post_user_name'] : '';
		}
	}
}
...
{
	$template->assign_block_vars('switch_user_name_select', array());
}
...
#	'BBCODE_STATUS' => sprintf($bbcode_status, '<a href="' . check_sid("faq.php?mode=bbcode") . '" target="_phpbbcode">', '</a>'), 
#	'SMILIES_STATUS' => $smilies_status, 
	...
	'U_VIEWTOPIC' => ( $mode == 'reply' ) ? check_sid("viewtopic.php?" . POST_TOPIC_URL . "=$topic_id&amp;postorder=desc") : '', 
	'U_REVIEW_TOPIC' => ( $mode == 'reply' ) ? check_sid("posting.php?mode=topicreview&amp;" . POST_TOPIC_URL . "=$topic_id") : '', 
...
#	'S_SMILIES_CHECKED' => ( !$smilies_on ) ? 'checked="checked"' : '', 
	...
	'S_TOPIC_ID' => $topic_id, 
	'S_POST_ACTION' => check_sid("posting.php"),
