...
class TSStatus
{
	...
	private function renderFlags($flags)
	{
		...
		{
			$img = (file_exists($this->imagePath . $flag)) ? $this->imagePath . $flag : $this->imagePath . '16x16_error.png';
		...
		try
		{
			...
			else 
			{
				...
				function zeitformat($Sekundenzahl)
				{
					...
					return sprintf("Uptime: %d Tage %02d:%02d:%02d", $Sekundenzahl/60/60/24,($Sekundenzahl/60/60)%24,($Sekundenzahl/60)%60,$Sekundenzahl%60);
				}
				#$einezeit = 187936;
				#echo zeitformat($einezeit);

				$out .= '<div><img src="' . $this->imagePath . '16x16_info.png" />' . zeitformat($this->_serverDatas["virtualserver_uptime"]) . "</a></div>\n";
				$out .= '<div><img src="' . $this->imagePath . '16x16_info.png" />'.substr($this->_serverDatas['virtualserver_version'], 0, stripos($this->_serverDatas['virtualserver_version'], ' ['))."</div>\n";
				$out .= '<div><img src="' . $this->imagePath . '16x16_info.png" />'.$this->unescape($this->_serverDatas['virtualserver_platform'])."</div>\n";
			#	$out .= '<div><img src="' . $this->imagePath . '16x16_info.png" />'.date("d.m.Y H:i", $this->unescape(substr($this->_serverDatas['virtualserver_version'], strpos($this->_serverDatas['virtualserver_version'], '[Build:') +7, -1)))."</div>\n";
				$out .= '<div><img src="' . $this->imagePath . '16x16_info.png" />'.date("d.m.Y H:i", $this->unescape($this->_serverDatas['virtualserver_created']))."</div>\n";
			#	$out .= $this->_serverDatas['virtualserver_created'];
