...
else
{
	...
	switch ( $mode )
	{
		...
		case '_update':
			...
			$s_fields = '<input type="hidden" name="mode" value="' . $mode . '" /><input type="hidden" name="' . POST_CASH_URL . '" value="' . $data_id . '" />';
		...
		case '_update_user':
			...
			$s_fields = '<input type="hidden" name="mode" value="' . $mode . '" /><input type="hidden" name="' . POST_CASH_USER_URL . '" value="' . $data_user . '" />';

			$template->assign_vars(array(
				'L_HEAD'			=> sprintf($lang['sprintf_head'], $lang['cash']),
				'L_INPUT'			=> sprintf($lang['sprintf' . $mode], $lang['user'], $data['user_id']),
				'L_USER'			=> $lang['user'],
			...
			if ( request('submit', 2) )
			{
				...
				$error = ( !$user_id ) ? $lang['msg_select_user'] : '';
		...
		case '_bankdata':
			...
			if ( request('submit', 2) )
			{
				...
				$error = ( !$bankdata_name ) ? $lang['msg_select_name'] : '';
				...
				if ( !$error )
				{
					...
					{			
						$sql = "INSERT INTO " . CASH_BANK . " (bankdata_name, bankdata_bank, bankdata_blz, bankdata_number, bankdata_reason) VALUES ('$bankdata_name', '$bankdata_bank', '$bankdata_blz', '$bankdata_number', '$bankdata_reason')";
					}
		...
		case '_bankdata_delete':
			...
			else if ( !$confirm )
			{
				...
				$template->assign_vars(array(
					...
					'MESSAGE_TEXT'	=> $lang['delete_confirm_bankdata'],
		...
		case '_delete_user':
			...
			else if ( $data_user && !$confirm )
			{
				...
				$s_fields = '<input type="hidden" name="mode" value="_user_delete" /><input type="hidden" name="' . POST_CASH_USER_URL . '" value="' . $data_user . '" />';
				
				$template->assign_vars(array(
					'MESSAGE_TITLE'	=> $lang['common_confirm'],
					'MESSAGE_TEXT'	=> sprintf($lang['sprintf_delete_confirm'], $lang['delete_confirm_cash_user'], $data['username']),
		...
		case '_delete':
			...
			else if ( $data_id && !$confirm )
			{
				...
				$s_fields = '<input type="hidden" name="mode" value="_delete" /><input type="hidden" name="' . POST_CASH_URL . '" value="' . $data_id . '" />';
	
				$template->assign_vars(array(
					'MESSAGE_TITLE'	=> $lang['common_confirm'],
					'MESSAGE_TEXT'	=> sprintf($lang['sprintf_delete_confirm'], $lang['delete_confirm_cash'], $data['cash_name']),
		...
		default:
			...
			$data_bank = get_data(CASH_BANK, '', 0);
			$data_cash = get_data_array(CASH, '', 'cash_id', 'ASC');
			...
			if ( $data_bank )
			{
				...
				$template->assign_vars(array(
					'NAME'		=> $data_bank['bankdata_name'],
					'BANK'		=> $data_bank['bankdata_bank'],
					'BLZ'		=> $data_bank['bankdata_blz'],
					'NUMBER'	=> $data_bank['bankdata_number'],
					'REASON'	=> $data_bank['bankdata_reason'],
				));
			}
			...
			if ( $data_cash )
			{
				for ( $i = $start; $i < min($settings['site_entry_per_page'] + $start, count($data_cash)); $i++ )
				{
					$cash_id	= $data_cash[$i]['cash_id'];	
					$cash_type	= $data_cash[$i]['cash_type'];
					
					if ( $data_cash[$i]['cash_interval'] == '0' )
					{
						$cash_amount	= $data_cash[$i]['cash_amount'];
						$cash_interval	= $lang['cash_interval_month'];
					}
					else if ( $data_cash[$i]['cash_interval'] == '1' )
					{
						$cash_amount	= 2 * str_replace(',', '.', $data_cash[$i]['cash_amount']);
						$cash_interval	= $lang['cash_interval_weeks'];
					}
					else if ( $data_cash[$i]['cash_interval'] == '2' )
					{
						$cash_amount	= 4 * str_replace(',', '.', $data_cash[$i]['cash_amount']);
						$cash_interval	= $lang['cash_interval_weekly'];
					}
					
					$template->assign_block_vars('_display._cash_row', array(
						'NAME'		=> $data_cash[$i]['cash_name'],
						'TYPE'		=> ( $cash_type != 0 ) ? ( $cash_type == '1' ? $images['sound'] : $images['other'] ) : $images['match'],
						'AMOUNT'	=> $data_cash[$i]['cash_amount'],
						'DATE'		=> $cash_interval,
						
						'U_UPDATE'	=> append_sid('admin_cash.php?mode=_update&amp;' . POST_CASH_URL . '=' . $cash_id),
			...
			$data_cashuser = $db->sql_fetchrowset($result);
			
			if ( $data_cashuser )
			{
				for ( $i = $start; $i < min($settings['site_entry_per_page'] + $start, count($data_cashuser)); $i++ )
				{
					$cashuser_id	= $data_cashuser[$i]['cash_user_id'];
					$month_cur		= date("m", time());
					$month_user		= explode(', ', $data_cashuser[$i]['user_month']);
					
					if ( $month_cur == $month_user || in_array($month_cur, $month_user) )
					{
						$postage_cashuser += $data_cashuser[$i]['user_amount'];
					}
					else if ( !$data_cashuser[$i]['user_interval'] && in_array($month_cur, $month_user) )
					{
						$postage_cashuser += $data_cashuser[$i]['user_amount'];
					}
					
					$user_interval = ( $data_cashuser[$i]['user_interval'] ) ? $lang['interval_only'] : $lang['interval_month'];
					
					$template->assign_block_vars('_display._cashuser_row', array(
						'USERNAME'	=> $data_cashuser[$i]['username'],
						'AMOUNT'	=> $data_cashuser[$i]['user_amount'],
						'INTERVAL'	=> $user_interval,
						
						'U_UPDATE'	=> append_sid('admin_cash.php?mode=_update_user&amp;' . POST_CASH_USER_URL . '=' . $cashuser_id),
						'U_DELETE'	=> append_sid('admin_cash.php?mode=_delete_user&amp;' . POST_CASH_USER_URL . '=' . $cashuser_id)
					));
				}
			}
