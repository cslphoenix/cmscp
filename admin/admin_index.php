...
else
{
	...
	add_lang(array('index', 'news', 'gallery'));
	...
	switch ( $mode )
	{
		...
		default:
			...
			else
			{
				...
				for ( $i = 0; $i < $cnt; $i++ )
				{
					...
					$template->assign_block_vars('news_row', array(
						...
						'PUBLIC'	=> ( $userauth['A_NEWS_PUBLIC'] )	? href('a_txt', 'index.php' . $iadds, array('mode' => 'switch', 'num' => $id), $public, '') : img('i_icon', 'icon_news_denied', ''),
			...
			$lastest_version = ( $lastest_checked != 'fsockopen' ) ? @implode('.', $lastest_checked) : 'NOT_WORKING';
