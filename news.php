...
=======
		...
		$sql = "SELECT c.*, u.user_id, u.user_name, u.user_color, u.user_email
					...
					WHERE c.type = " . READ_NEWS . "
				ORDER BY c.time_create ASC";
		if ( !($result = $db->sql_query($sql)) )
>>>>>>> .r85
		{
<<<<<<< .mine
...
<<<<<<< .mine
=======
			#	( $unread['read_time'] < $comments[$i]['time_create'] ) ? sql(COMMENT_READ, 'create', array('type' => READ_MATCH, 'type_id' => $data, 'user_id' => $user, 'read_time' => $comments[$i]['time_create'])) : sql(COMMENT_READ, 'update', array('read_time' => $comments[$i]['time_create']), array('type', 'type_id', 'user_id'), array(READ_MATCH, $data, $user));
			}
...
=======
			...
			( !$unread ) ? sql(COMMENT_READ, 'create', array('user_id' => $user, 'type_id' => $data, 'type' => READ_MATCH, 'read_time' => $time)) : sql(COMMENT_READ, 'update', array('read_time' => $time), array('type', 'type_id', 'user_id'), array(READ_MATCH, $data, $user));
>>>>>>> .r85
		}
...
<<<<<<< .mine
				...
				( !$unread ) ? sql(COMMENT_READ, 'create', array('type' => READ_MATCH, 'type_id' => $data, 'user_id' => $user, 'read_time' => $comments[$i]['time_create'])) : sql(COMMENT_READ, 'update', array('read_time' => $comments[$i]['time_create']), array('type', 'type_id', 'user_id'), array(READ_MATCH, $data, $user));
			}
			...
			$template->assign_vars(array(
				'L_GOTO_PAGE'	=> $lang['Goto_page'],
				'PAGINATION'	=> generate_pagination("$file?mode=view&amp;$url=$data", count($comments), $settings['site_comment_per_page'], $start),
				'PAGE_NUMBER'	=> sprintf($lang['common_page_of'], ( floor( $start / $settings['site_comment_per_page'] ) + 1 ), $current_page ),
			));
